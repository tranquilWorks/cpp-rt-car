...
int main() {
    ...
    for(const auto& c:b::cpu::cases()) {
        if(c.kind==b::cpu::Kind::compile || c.kind==b::cpu::Kind::memory || c.kind==b::cpu::Kind::lifecycle) continue;
        b::cpu::Provider p;if(p.prepare(c.id)!=b::Status::ok) return 1;
    ...
    std::cout<<"CPU steady-state allocation checks passed\n";
}
