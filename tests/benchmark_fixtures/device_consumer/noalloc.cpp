...
int main() {
    ...
    for(const auto& c:b::device::cases()) {
        if(c.real || !c.allocation_free) continue;
        b::device::Provider p;if(p.prepare(c.id)!=b::Status::ok) return 1;
        ...
        for(std::uint64_t ordinal=0;ordinal<7;++ordinal) {
            // Includes the complete backend transaction on any prestarted I/O workers.
    ...
    std::cout<<"Device steady-state allocation checks passed\n";
}
